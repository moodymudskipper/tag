...
Short names have been favored so far so more visual focus is on the tagged 
object (it is also shorter to type!)
...
Trhough the `.key` argument `summarize` also supports multi spread in an
...
Tags are an exploration methods for `$`. 

They could have been just `NA` objects with a method for `$`, this is the approach
taken by Gabor Grothendieck for `gsubfn::fn`. I think however that it's clearer
to think of them as functions with a `$` shorthand thanks to their class `"tag"`.
...
are left untouched or some are removed (e.g. `partial`). i.e. We find a lot of
...
Traditional adverbs suffer from their naming inconsistencies :

* compose, partial, safely, Negate
* rate_delay is an adverb, rate_backoff isn't
* brackets and parameters are awkward for inline use :
* the adverb is highlighted, the function is not
...
Through some miracle that I don't understand, autocomplete works on tagged
functions.
